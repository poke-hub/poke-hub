...
class TwoFactorLoginBehavior(TaskSet):
    ...
    def login_2fa_flow(self):
        ...
        with self.client.get(
            "/login", 
            name="/login (2FA Step 1 GET)",
            catch_response=True
        ) as response_get_login:
        ...
        with self.client.post(
            ...
            if response_post_login.status_code != 302 or response_post_login.headers.get('Location') != "/login/verify-2fa":
                response_post_login.failure(f"Step 1 (Password) failed. Expected 302 to /login/verify-2fa, got {response_post_login.status_code}")
                return

        with self.client.get(
            "/login/verify-2fa", 
            name="/login/verify-2fa (2FA Step 2 GET)",
            catch_response=True
        ...
        with self.client.post(
            ...
            if response_post_verify.status_code != 302 or response_post_verify.headers.get('Location') != "/":
                response_post_verify.failure(f"Step 2 (Token) failed. Expected 302 to /, got {response_post_verify.status_code}")
                return
