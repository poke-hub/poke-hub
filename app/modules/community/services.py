...
class CommunityService(BaseService):
    ...
    def get_available_user_datasets_for_proposal(user, community):
        requested_ids = {
            req.dataset_id
            for req in CommunityDatasetRequest.query
                .filter_by(community_id=community.id, status="pending")
                .all()
        }

        return [
            ds for ds in user.data_sets
            if ds.community_id != community.id and ds.id not in requested_ids
        ]
